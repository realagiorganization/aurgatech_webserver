...
namespace aurga
{
    public static class Endpoint_V1
    {
        public static void MapV1Endpoints(this WebApplication app)
        {
            ...
            #region User Sign Up
            app.MapPost("/register/user", async (HttpContext http, ReuqestRegisterUser user) =>
            {
                try
                {
                    // 
                    ...
                    var response = await httpClient.PostAsync($"{SharedStore.MIRROR_URL}/register/user", content);
            ...
            app.MapPost("/login", async (HttpContext http, LoginRequest request) =>
            {
                try
                {
                    ...
                    var response = await httpClient.PostAsync($"{SharedStore.MIRROR_URL}/login", content);
                    var responseString = await response.Content.ReadAsStringAsync();
                    ...
                    var responseData = new { status = 0, uid = userInfo.UserGUID, token = userToken, v = 1, payload = Util.Bytes2Hex(payload) };
            ...
            // Deprecated
            app.MapPost("/loginWithToken", async (HttpContext http, RequestLoginWithToken request) =>
            {
                try
                {
#if MIRROR
                    var httpClient = new HttpClient();
                    var content = new StringContent(JsonSerializer.Serialize(request), Encoding.UTF8, "application/json");
                    var response = await httpClient.PostAsync($"{SharedStore.MIRROR_URL}/loginWithToken", content);
                    var responseString = await response.Content.ReadAsStringAsync();
                    ...
                    lock (cacheUsers)
                    {
                        ...
                        var responseData = new { status = RC.SUCCESS, uid = userInfo.UserGUID, token = userToken, v = 1, payload = Util.Bytes2Hex(payload) };
                        return Results.Json(responseData);
                    }
                }
                catch (Exception e)
